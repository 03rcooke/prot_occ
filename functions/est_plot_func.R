...
est_plot_func <- function(grp, est_df, est_diff, vari, lim = NULL, axlab) {
  ...
  est_plot <- ggplot2::ggplot(df, aes(x = prot, y = .data[[vari]])) +
    # median lines
    ggplot2::geom_hline(data = meds, aes(yintercept = meds, colour = prot), lty = 2, lwd = 1) +
    # jittered points
    ggplot2::geom_jitter(aes(colour = prot), alpha = 0.4)  +
    # scales
  ...
  est_diff_plot <- ggplot2::ggplot(diff_df, aes(x = "", y = .data[[vari_diff]])) +
    # median lines
    ggplot2::geom_hline(data = meds, aes(yintercept = diff), colour = meds$colr, lty = 2, lwd = 1) +
    # half violin
    gghalves::geom_half_violin(aes(fill = .data[[vari_sig]]), side = "r", colour = NA, scale = "count", alpha = 0.6) +
    # 95% CrI
    ggplot2::geom_linerange(data = diff_av, aes(ymax = .data[[paste0(vari_diff, "_upp_ci")]], ymin = .data[[paste0(vari_diff, "_low_ci")]], x = "", colour = .data[[vari_sig]]), size = 1, inherit.aes = FALSE) +
    # median
    ggplot2::geom_point(data = diff_av, aes(y = .data[[paste0(vari_diff, "_median")]], colour = .data[[vari_sig]]), size = 10, shape = "-") +
    # relative effect size
    ...
    ggplot2::scale_fill_manual(values = c("#33BBEE", "grey50", "#009988"), drop = FALSE) +
    ggplot2::scale_colour_manual(values = c("#33BBEE", "grey50", "#009988"), drop = FALSE) +
    ggplot2::labs(x = "") +
    ggplot2::theme(legend.position = "none",
