...
trend_plot_func <- function(grp, msi_sum, lim = NULL, leg = FALSE, leg_coord = NULL, axlab = "Geometric mean occupancy") {
  ...
  trend_plot <- ggplot2::ggplot(data = msi, aes(x = year, y = median, group = prot)) +
    ggplot2::geom_hline(data = refe, aes(colour = prot, yintercept = median), lty = 2) +
    # 95% CrI
    ggplot2::geom_ribbon(aes(ymin = low_ci, ymax = upp_ci, fill = prot), alpha = 0.6) +
    ggplot2::geom_line(aes(y = median, colour = prot)) +
    ggplot2::geom_point(aes(colour = prot), size = 0.6) +
    # scales
    ggplot2::scale_y_continuous(name = axlab, limits = c(low_lim, upp_lim)) +
    ggplot2::scale_x_continuous(name = "Year", breaks = scales::pretty_breaks(6), expand = c(0,0)) +
    ggplot2::scale_colour_manual(name = "", values = c("#0077BB", "#EE7733")) +
    ggplot2::scale_fill_manual(name = "", values = c("#5DC4FF", "#F6BB99")) +
    ggplot2::theme(legend.position = "none")
  
  if(leg == TRUE) trend_plot <- trend_plot + 
    ggplot2::annotate("text", x = leg_coord[1], y = leg_coord[2], label = "Protected", colour = "#0077BB", size = 6) + 
    ggplot2::annotate("text", x = leg_coord[3], y = leg_coord[4], label = "Unprotected", colour = "#EE7733", size = 6) 
...
# # save plot
# cowplot::save_plot("outputs/fig3_chng_comb_over.png", chng_comb_over, base_height = 11, base_width = 9, dpi = 300)
...
# # save plot
# cowplot::save_plot("outputs/fig4_beta_over.png", beta_over_plot, base_height = 8, base_width = 12, dpi = 300)
...
# # save plot
# cowplot::save_plot("outputs/fig5_poll.png", poll_plot, base_height = 12, base_width = 10, dpi = 300)
...
# # save plot
# cowplot::save_plot("outputs/figs10_pred.png", pred_plot, base_height = 12, base_width = 10, dpi = 300)
...
# # save: pro_sub_plot
# cowplot::save_plot("outputs/fig1_pro_sub.png", pro_sub_plot, base_height = 6, base_width = 6, dpi = 300)
...
# # save plot
# cowplot::save_plot("outputs/figs9_conv.png", conv_plot, base_height = 12, base_width = 10, dpi = 300)
...
# protected
maps_pers_tax_pa <- assessSpatialCov_adapt(dat = dplyr::filter(all_obs, prot == "pa"), periods = periods, res = 1000, logCount = TRUE, species = "species", x = "EASTING", y = "NORTHING", year = "year", spatialUncertainty = "spatialUncertainty", identifier = "tax_grp", output = "nPeriods", sites_gr = sites_gr, pa = TRUE) # adapted occAssess::assessSpatialCov
...
maps_pers_tax_pa_func <- function(x, leg = FALSE) {
  ...
  maps_out <- maps_pers_tax_pa[[x]] +
    ...
    ggplot2::geom_text(x = 550000, y = 600000, label = max(maps_pers_tax_pa[[x]]$data$value, na.rm = TRUE), size = 3) +
    ...
    # edit theme
    ggplot2::theme(axis.ticks = element_blank(), axis.text = element_blank(), axis.title = element_blank(), panel.border = element_blank(), panel.grid = element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank())
...
maps_pers_tax_unp_func <- function(x, leg = FALSE) {
  ...
  maps_out <- maps_pers_tax_unp[[x]] +
    ggplot2::coord_equal() +
    # add maximum repeated value
    ggplot2::geom_text(x = 550000, y = 600000, label = max(maps_pers_tax_unp[[x]]$data$value, na.rm = TRUE), size = 3) +
    # change colour scheme
...
# generate legend
maps_pers_leg <- cowplot::get_legend(maps_pers_tax_pa_func(3) + ggplot2::scale_fill_manual(values = colrs_nam[2:max(maps_pers_tax_pa[[3]]$data$value, na.rm = TRUE)], na.value = "white", name = "Number of\nyears sampled", drop = FALSE) + ggplot2::theme(legend.position = "bottom", legend.direction = "horizontal"))
...
maps_pers_tax_comb_leg2 <- cowplot::ggdraw(maps_pers_tax_comb_leg) +
  ggplot2::theme(plot.background = element_rect(fill = "white", color = NA))

# # save plot
# cowplot::save_plot("outputs/figs1_maps_pers_tax.png", maps_pers_tax_comb_leg, base_height = 12, base_aspect_ratio = 0.7, dpi = 300)
...
maps_tax_pa_func <- function(x, leg = TRUE) {
  ...
  maps_out <- maps_tax_pa[[x]] +
    # change period names
    ggplot2::facet_wrap(. ~ variable, labeller = period_labeller) +
    # change fill colour
...
# ants, bees
maps_ab <- cowplot::plot_grid(maps_tax_pa_out[[1]] + ggplot2::labs(title = "Ants"), maps_tax_unp_out[[1]] + ggplot2::labs(title = "Ants"), maps_tax_pa_out[[2]] + ggplot2::labs(title = "Bees"), maps_tax_unp_out[[2]] + ggplot2::labs(title = "Bees"), ncol = 2)

# cowplot::save_plot("outputs/figs2_maps_ab.png", maps_ab, base_height = 12, base_aspect_ratio = 0.7, dpi = 300)

# hoverflies, ladybirds
maps_hl <- cowplot::plot_grid(maps_tax_pa_out[[3]] + ggplot2::labs(title = "Hoverflies"), maps_tax_unp_out[[3]] + ggplot2::labs(title = "Hoverflies"), maps_tax_pa_out[[4]] + ggplot2::labs(title = "Ladybirds"), maps_tax_unp_out[[4]] + ggplot2::labs(title = "Ladybirds"), ncol = 2)

# cowplot::save_plot("outputs/figs3_maps_hl.png", maps_hl, base_height = 12, base_aspect_ratio = 0.7, dpi = 300)

# spiders, wasps
maps_sw <- cowplot::plot_grid(maps_tax_pa_out[[5]] + ggplot2::labs(title = "Spiders"), maps_tax_unp_out[[5]] + ggplot2::labs(title = "Spiders"), maps_tax_pa_out[[6]] + ggplot2::labs(title = "Wasps"), maps_tax_unp_out[[6]] + ggplot2::labs(title = "Wasps"), ncol = 2)

# cowplot::save_plot("outputs/figs4_maps_sw.png", maps_sw, base_height = 12, base_aspect_ratio = 0.7, dpi = 300)
...
spat_tax_func <- function(tax) {
  ...
  spatBias_tax_out <- ggplot2::ggplot(data = spat_dat_tax, aes(x = year, y = mean, ymin = lower, ymax = upper, colour = identifier, fill = identifier)) + 
    ggplot2::geom_hline(yintercept = 1, colour = "grey", lty = 2) +
    ggplot2::geom_line() + 
    ggplot2::geom_point() + 
    ggplot2::geom_ribbon(alpha = 0.5) + 
    # mean value across years
    ggplot2::geom_text(x = 1992, y = 1.3, label = sprintf("%.2f", signif(mean(dplyr::filter(spat_dat_tax, identifier == "Protected")$mean), digits = 2)), colour = "#0077BB", size = 3) +
    ggplot2::geom_text(x = 1992, y = 1.1, label = sprintf("%.2f", signif(mean(dplyr::filter(spat_dat_tax, identifier == "Unprotected")$mean), digits = 2)), colour = "#EE7733", size = 3) +
    ggplot2::theme_linedraw() + 
    ggplot2::scale_colour_manual(values = c("#0077BB", "#EE7733")) +
    ggplot2::scale_fill_manual(values = c("#0077BB", "#EE7733")) +
    ggplot2::labs(title = tax, x = "Year", y = "Nearest neighbour index", colour = "", fill = "") +
...
spatBias_tax_leg <- cowplot::get_legend(spatBias_tax[[1]] + ggplot2::theme(legend.position = "bottom"))

spatBias_tax_comb <- cowplot::plot_grid(spatBias_tax[[1]] + ggplot2::theme(legend.position = "none"), spatBias_tax[[2]] + ggplot2::theme(legend.position = "none") + ggplot2::labs(y = ""), spatBias_tax[[3]] + ggplot2::theme(legend.position = "none"), spatBias_tax[[4]] + ggplot2::theme(legend.position = "none") + ggplot2::labs(y = ""), spatBias_tax[[5]] + ggplot2::theme(legend.position = "none"), spatBias_tax[[6]] + ggplot2::theme(legend.position = "none") + ggplot2::labs(y = ""), ncol = 2)
...
cowplot::save_plot("occ_assess/figs5_spat_bias_tax.png", spatBias_tax_comb2, base_height = 8, base_aspect_ratio = 1, dpi = 300)
...
rep_plot <- function(tax, pa = NULL) {
  ...
  if(pa == TRUE) {
    ...
    p <- p + 
    # add protected areas
      ggplot2::geom_tile(data = dplyr::filter(sites_gr, prot == "pa"), aes(x = EASTING, y = NORTHING), alpha = 0.5, fill = "#0077BB", colour = "#0077BB", lwd = 0)
  ...
  if(pa == FALSE) {
    ...
    p <- p + 
      ...
      ggplot2::geom_tile(data = dplyr::filter(sites_gr, prot == "unp"), aes(x = EASTING, y = NORTHING), alpha = 0.5, fill = "#EE7733", colour = "#EE7733", lwd = 0)
    
  }
  
  p <- p +
    # grid cells with repeat visits
    ggplot2::geom_tile(fill = "black") +
    ggplot2::coord_equal() +
    ggplot2::labs(title = tax) +
    # split into periods
    ggplot2::facet_wrap(~ perd_7) +
    ggplot2::theme_linedraw() +
    # edit theme
    ggplot2::theme(axis.ticks = element_blank(), axis.text = element_blank(), axis.title = element_blank(), panel.border = element_blank(), panel.grid = element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank())
...
cowplot::save_plot("outputs/figs6_rep_ab.png", rep_ab, base_height = 12, base_aspect_ratio = 0.5, dpi = 300)

# hoverflies, ladybirds
rep_hl <- cowplot::plot_grid(rep_maps_pa[[3]], rep_maps_unp[[3]], rep_maps_pa[[4]], rep_maps_unp[[4]], ncol = 2)

cowplot::save_plot("outputs/figs7_rep_hl.png", rep_hl, base_height = 12, base_aspect_ratio = 0.5, dpi = 300)

# spiders, wasps
rep_sw <- cowplot::plot_grid(rep_maps_pa[[5]], rep_maps_unp[[5]], rep_maps_pa[[6]], rep_maps_unp[[6]], ncol = 2)

cowplot::save_plot("outputs/figs8_rep_sw.png", rep_sw, base_height = 12, base_aspect_ratio = 0.5, dpi = 300)
