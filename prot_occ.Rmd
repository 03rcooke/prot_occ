...
source("/data/notebooks/rstudio-proareas/proarea/pa_pr_fun.R")
...
# new iteration numbers - old ones produce weird results
set.seed(186875)
rseq <- as.vector(replicate(nrow(occ_pa_prep) / 999, sample(1:999, 999, replace = FALSE)))

occ_pa_prep <- occ_pa_prep %>% 
  dplyr::mutate(iteration = rseq)

occ_unp_prep <- occ_unp_prep %>% 
  dplyr::mutate(iteration = rseq)
...
# tidy up
rm(all_spp, hov_pa, hov_unp, occ_pa_func, occ_pa_prep, occ_unp_func, occ_unp_prep, spp_pa, spp_unp); gc()
...
est_plot_func <- function(grp, est_df, est_diff, vari, lim = NULL, axlab) {
  ...
  est_prep_plot <- cowplot::plot_grid(est_plot, est_diff_plot, rel_widths = c(1, 0.8)) #%>% 
    #cowplot::plot_grid(est_title, ., nrow = 2, rel_heights = c(0.08, 1))
...
# # save plots
# cowplot::save_plot("outputs/sprich_over.png", spr_over[[1]], base_height = 6, base_width = 8, dpi = 600)
...
# # save plot
# cowplot::save_plot("outputs/chng_comb_over.png", chng_comb_over, base_height = 11, base_width = 9, dpi = 600)
...
# beta_comb_plot <- cowplot::plot_grid(NULL, beta_over[[1]], NULL, nrow = 1, rel_widths = c(0.4, 1, 0.4), labels = c("", "A", "")) %>% cowplot::plot_grid(., cowplot::plot_grid(beta_poll[[1]], beta_pred[[1]], nrow = 1, labels = c("B", "C")), nrow = 2)
...
# loss_comb_plot <- cowplot::plot_grid(NULL, loss_over[[1]], NULL, nrow = 1, rel_widths = c(0.4, 1, 0.4), labels = c("", "A", "")) %>% cowplot::plot_grid(., cowplot::plot_grid(loss_poll[[1]], loss_pred[[1]], nrow = 1, labels = c("B", "C")), nrow = 2)
...
# gain_comb_plot <- cowplot::plot_grid(NULL, gain_over[[1]], NULL, nrow = 1, rel_widths = c(0.4, 1, 0.4), labels = c("", "A", "")) %>% cowplot::plot_grid(., cowplot::plot_grid(gain_poll[[1]], gain_pred[[1]], nrow = 1, labels = c("B", "C")), nrow = 2)
...
# # save plot
# cowplot::save_plot("outputs/beta_over.png", beta_over_plot, base_height = 8, base_width = 12, dpi = 600)
...
# # save plot
# cowplot::save_plot("outputs/poll.png", poll_plot, base_height = 12, base_width = 10, dpi = 600)
...
# # save plot
# cowplot::save_plot("outputs/pred.png", pred_plot, base_height = 12, base_width = 10, dpi = 600)
...
# # save: pro_plot
# cowplot::save_plot("outputs/pro.png", pro_plot, base_height = 5.8, base_width = 5, dpi = 300)
...
# # save: pro_sub_plot
# cowplot::save_plot("outputs/pro_sub.png", pro_sub_plot, base_height = 6, base_width = 6, dpi = 600)

# # for occAssess
# 
# saveRDS(sites_gr, "data/sites_gr.rds")
...
ed_obs <- obs(tax_path = "/data-s3/occmods/E&D/occmod_outputs/2021_Francesca/", tax_grp = "E&D", filetype = "rdata")

# saveRDS(ed_obs, "data/ed_obs.rds")
...
all_obs <- dplyr::bind_rows(ants_obs, bees_obs, ed_obs, hover_obs, lady_obs, spider_obs, wasps_obs)
...
out_dat <- load_rdata(paste0("/data-s3/occmods/Ants/occmod_outputs/2021_Francesca_bwars_rerun/", "Formica exsecta", ".rdata"))

bugs <- out_dat$BUGSoutput$summary %>% 
  data.frame() %>% 
  tibble::rownames_to_column("para") %>% 
  dplyr::filter(stringr::str_detect(para, "psi.fs")) %>% 
  dplyr::filter(!stringr::str_detect(para, "psi.fs.r_ENGLAND|psi.fs.r_GB|psi.fs.r_SCOTLAND|psi.fs.r_UK|psi.fs.r_WALES|psi.fs.r_high|psi.fs.r_low|psi.fs.r_no_agri|psi.fs.r_pa|psi.fs.r_unp"))
...
ed_rhat <- rhat(tax_path = "/data-s3/occmods/E&D/occmod_outputs/2021_Francesca/", tax_grp = "E&D", filetype = "rdata")

# saveRDS(ed_rhat, "data/ed_rhat.rds")
...
all_rhat <- dplyr::bind_rows(ants_rhat, bees_rhat, ed_rhat, hover_rhat, lady_rhat, spider_rhat, wasps_rhat)

# saveRDS(all_rhat, "data/all_rhat.rds")

all_rhat <- readRDS("data/all_rhat.rds") %>% 
  dplyr::filter(tax_grp %in% taxa)
...
# # save plot
# cowplot::save_plot("outputs/chng_comb_over_conv.png", chng_comb_over_conv, base_height = 12, base_width = 9, dpi = 300)
...
# # save plot
# cowplot::save_plot("outputs/conv.png", conv_plot, base_height = 12, base_width = 10, dpi = 600)
